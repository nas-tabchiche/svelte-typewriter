...
var require_loopTypewriter = __commonJS((exports2) => {
  ...
  var loopTypewriter = async ({currentNode, text: text2}, options) => {
    await writeEffect({currentNode, text: text2}, options);
  ...
  var mode = async (elements, options) => {
    while (true) {
      if (options.loop) {
        ...
        const element2 = getRandomElement(elements);
        await loopTypewriter(element2, options);
      }
    }
  };
...
var require_getLongestTextElement = __commonJS((exports2) => {
  __export(exports2, {
...
var require_typewriter = __commonJS((exports2) => {
  ...
  var mode = async (elements, options) => {
    if (options.cascade) {
      ...
      const {getLongestTextElement} = await Promise.resolve().then(() => __toModule(require_getLongestTextElement()));
      const lastElementToFinish = getLongestTextElement(elements);
      onAnimationEnd(lastElementToFinish, () => options.dispatch("done"));
    }
    for (const element2 of elements)
      options.cascade ? await writeEffect(element2, options) : writeEffect(element2, options);
...
var import_sleep = __toModule(require_sleep());

// src/helpers/rng.ts
var rng = (min, max) => Math.floor(Math.random() * (max - min) + min);

// src/helpers/typingInterval.ts
var typingInterval = async (interval) => import_sleep.sleep(Array.isArray(interval) ? interval[rng(0, interval.length)] : interval);
...
var typewriter = async (node, options) => {
  ...
  if (options.delay > 0) {
    const {sleep: sleep2} = await Promise.resolve().then(() => __toModule(require_sleep()));
    await sleep2(options.delay);
